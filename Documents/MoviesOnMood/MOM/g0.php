...
$id=$_GET["id"];
	...
	if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		$rate=$_POST["rate"];
		if((stristr($comments,"good")!==false || stristr($comments,"amazing")!==false || stristr($comments,"loved")!==false || stristr($comments,"awesome")!==false || stristr($comments,"great")!==false) && $rate<4){
			echo '<script>alert("Enter Suitable Rating Matching Comment or vice versa.\nFor ex:-\nGood movie Rating >=4\nBad movie Rating < 4")</script>'; 
		}
		
		else if((stristr($comments,"bad")!==false || stristr($comments,"worst")!==false || stristr($comments,"not good")!==false || stristr($comments,"money")!==false) && $rate>3){
			echo '<script>alert("Enter Suitable Rating Matching Comment or vice versa.\nFor ex:-\nGood movie Rating >=4\nBad movie Rating < 4")</script>'; 
		}
		else{
...
<body>
	...
	<div class="m">
		<div class="main f">
			...
			<p>
				...
				<p class='pad'>
					...
					<a href="https://www.netflix.com/in/"><img class="logos" src="https://cdn.vox-cdn.com/thumbor/lfpXTYMyJpDlMevYNh0PfJu3M6Q=/39x0:3111x2048/920x613/filters:focal(39x0:3111x2048):format(webp)/cdn.vox-cdn.com/uploads/chorus_image/image/49901753/netflixlogo.0.0.png">
					</a>
					<a href="https://www.primevideo.com/?ref_=dvm_pds_amz_in_as_s_b_brand1|m_lgAX6a65c_c"><img class="logos" src="https://www.mediaplaynews.com/wp-content/uploads/2018/04/Prime-Video-Stacked.jpg">
					</a>
					</p>
				</p>
		...
		<div class="main fi">
			<form action="g0.php?id=<?php echo $id;?>" method="post">
			<td>
			<h2>Comments:</h2>
            ...
            if($result = mysqli_query($conn, $sql)){
                ...
                while($row = mysqli_fetch_assoc($result)){
                	...
                	if(strlen($row['comments'])>100){
                   		$b=$row['comments'];
                		echo "<tr>";
                    	echo "<td>".$row['name']."  (Rating :".$row['rate'].")"."<br>' ";
                    	while(strlen($b)>100){
                    		$a=substr($b, 0,100);
                    		$b=substr($b,100);
                    		echo $a."<br>";
